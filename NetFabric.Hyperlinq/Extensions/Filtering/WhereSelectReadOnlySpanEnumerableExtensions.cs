...
namespace NetFabric.Hyperlinq
{
    public static partial class WhereSelectReadOnlySpanEnumerableExtensions
    {
        ...
        public static TResult[] ToArray<TSource, TResult, TPredicate, TSelector>(this WhereSelectReadOnlySpanEnumerable<TSource, TResult, TPredicate, TSelector> source)
            where TPredicate : struct, IFunction<TSource, bool>
            where TSelector : struct, IFunction<TSource, TResult>
        {
            ...
            try
            {
                ...
                foreach (var item in span)
                {
                    if (predicate.Invoke(item))
                    {
                        ...
                        buffer[count++] = selector.Invoke(item);
                    }
                }
