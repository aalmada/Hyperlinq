...
namespace NetFabric.Hyperlinq.Analyzer
{
    ...
    public class AvoidLargeValueDelegateAnalyzer : DiagnosticAnalyzer
    {
        ...
        private static void AnalyzeStructDeclaration(SyntaxNodeAnalysisContext context)
        {
            ...
            // Check if ALREADY implements IFunctionIn
        ...
        private static int EstimateSize(ITypeSymbol type)
        {
            ...
            switch (type.SpecialType)
            {
                ...
                case SpecialType.System_String: return 8; // Reference (64-bit)
                case SpecialType.System_Object: return 8; // Reference
                case SpecialType.None:
                    if (type.IsReferenceType) return 8;
                    if (type.TypeKind == TypeKind.Enum && type is INamedTypeSymbol enumType)
                        return EstimateSize(enumType.EnumUnderlyingType ?? type); // Should allow recursion for underlying?
                        
                    // Recursively sum fields for structs
                    if (type.TypeKind == TypeKind.Struct && type is INamedTypeSymbol structSymbol)
                    {
                        ...
                        foreach (var member in structSymbol.GetMembers().OfType<IFieldSymbol>())
                        {
                            ...
                            {
                                // Fixed buffers logic omitted for simplicity unless needed
                                // Assuming sequential/auto layout
                                sum += EstimateSize(member.Type); 
                            }
                        }
                        return sum == 0 ? 1 : sum; // Empty struct is 1 byte
                    }
