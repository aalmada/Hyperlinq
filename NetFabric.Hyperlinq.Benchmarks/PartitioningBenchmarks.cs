...
namespace NetFabric.Hyperlinq.Benchmarks
{
    ...
    public class PartitioningBenchmarks
    {
        ...
        public int[] Skip_ToArray_LINQ()
        {
            return Enumerable.ToArray(Enumerable.Skip(array, Count / 4));
        }

        [BenchmarkCategory("Skip_ToArray"), Benchmark]
        public int[] Skip_ToArray_Hyperlinq()
        {
            return array.AsSpan().Skip(Count / 4).ToArray();
        }

        // ===== Take_ToArray =====

        [BenchmarkCategory("Take_ToArray"), Benchmark(Baseline = true)]
        public int[] Take_ToArray_LINQ()
        {
            return Enumerable.ToArray(Enumerable.Take(array, Count / 2));
        }

        [BenchmarkCategory("Take_ToArray"), Benchmark]
        public int[] Take_ToArray_Hyperlinq()
        {
            return array.AsSpan().Take(Count / 2).ToArray();
        }

        // ===== SkipTake_ToArray =====

        [BenchmarkCategory("SkipTake_ToArray"), Benchmark(Baseline = true)]
        public int[] SkipTake_ToArray_LINQ()
        {
            return Enumerable.ToArray(Enumerable.Take(Enumerable.Skip(array, Count / 4), Count / 4));
        }

        [BenchmarkCategory("SkipTake_ToArray"), Benchmark]
        public int[] SkipTake_ToArray_Hyperlinq()
        {
            return array.AsSpan().Skip(Count / 4).Take(Count / 4).ToArray();
        }
