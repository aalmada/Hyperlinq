...
public class AsValueEnumerableBenchmarks
{
    ...
    public int Array_Hyperlinq_Sum() => array.Sum();  // Hyperlinq extension

    // ===== Array_WhereSelectSum =====

    [BenchmarkCategory("Array_WhereSelectSum"), Benchmark(Baseline = true)]
    public int Array_LINQ_WhereSelectSum() => Enumerable.Sum(Enumerable.Select(Enumerable.Where(array, x => x % 2 == 0), x => x * 2));

    [BenchmarkCategory("Array_WhereSelectSum"), Benchmark]
    public int Array_Hyperlinq_WhereSelectSum() => array.AsValueEnumerable().Where(x => x % 2 == 0).Select(x => x * 2).Sum();  // Hyperlinq extensions

    // ===== IEnumerable_Sum =====

    [BenchmarkCategory("IEnumerable_Sum"), Benchmark(Baseline = true)]
    public int IEnumerable_LINQ_Sum() => Enumerable.Sum(enumerable);

    [BenchmarkCategory("IEnumerable_Sum"), Benchmark]
    public int IEnumerable_Hyperlinq_Sum() => enumerable.AsValueEnumerable().Sum();
    ...
    public bool List_Hyperlinq_Any() => list.Any();  // Hyperlinq extension
    ...
    public int List_Hyperlinq_Count() => list.Count();  // Hyperlinq extension (property access)
    ...
    public int List_Hyperlinq_Sum() => list.Sum();  // Hyperlinq extension

    // ===== List_WhereSelectSum =====

    [BenchmarkCategory("List_WhereSelectSum"), Benchmark(Baseline = true)]
    public int List_LINQ_WhereSelectSum() => Enumerable.Sum(Enumerable.Select(Enumerable.Where(list, x => x % 2 == 0), x => x * 2));

    [BenchmarkCategory("List_WhereSelectSum"), Benchmark]
    public int List_Hyperlinq_WhereSelectSum() => list.AsValueEnumerable().Where(x => x % 2 == 0).Select(x => x * 2).Sum();  // Hyperlinq extensions
    ...
    public int IEnumerable_LINQ_Count() => Enumerable.Count(enumerable);

    [BenchmarkCategory("IEnumerable_Count"), Benchmark]
    public int IEnumerable_Hyperlinq_Count() => enumerable.AsValueEnumerable().Count();

    // ===== IEnumerable_Any =====

    [BenchmarkCategory("IEnumerable_Any"), Benchmark(Baseline = true)]
    public bool IEnumerable_LINQ_Any() => Enumerable.Any(enumerable);

    [BenchmarkCategory("IEnumerable_Any"), Benchmark]
    public bool IEnumerable_Hyperlinq_Any() => enumerable.AsValueEnumerable().Any();
}
